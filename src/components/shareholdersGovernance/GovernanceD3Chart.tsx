...
const GovernanceD3Chart: React.FC<GovernanceD3ChartProps> = ({ onNodeHover, onNodeClick }) => {
  ...
  const draw = useCallback((svg: d3.Selection<SVGSVGElement, unknown, null, undefined>) => {
    ...
    // 로고 삽입 헬퍼 (폰트 로드 후 정확한 텍스트 너비 사용)
    const placeLogos = () => {
      nodes.each(function(d) {
        if (!d.logo) return;
        const grp = d3.select(this);
        grp.selectAll('image').remove(); // 중복 방지
        const t = grp.select<SVGTextElement>('text.gov-nd-name');
        const tNode = t.node();
        if (!tNode) return;
        const tw = tNode.getComputedTextLength();
        // tw가 0이면 한국어 기준 글자수×fontSize로 추정
        const estTw = tw > 2 ? tw : d.name.length * 22 * 0.9;
        let lh = 28 * (d.lscale ?? 1);
        let lw = lh * (d.lar ?? 1);
        const gap = 7;
        const maxRow = d.w - 22;
        if (lw + gap + estTw > maxRow) {
          const s = (maxRow - gap - estTw) / lw;
          lh = Math.max(10, lh * s);
          lw = lh * (d.lar ?? 1);
        }
        const startX = (d.w - (lw + gap + estTw)) / 2;
        const hasSub = d.sub && d.sub !== '비외감기업';
    ...
    if ('fonts' in document) {
      document.fonts.ready.then(placeLogos);
    } else {
      setTimeout(placeLogos, 200);
    }
